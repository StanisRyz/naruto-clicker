...
- To adjust ProgressInfoPanel (zone name, enemies count, enemy name, HP bar text, boss timer): change the dedicated `PROGRESS_*` constants — `PROGRESS_ZONE_FONT_SIZE`, `PROGRESS_ENEMIES_FONT_SIZE`, `PROGRESS_ENEMY_NAME_FONT_SIZE`, `PROGRESS_HP_TEXT_FONT_SIZE`, `PROGRESS_BOSS_TIMER_FONT_SIZE`. These control only `ProgressInfoPanel` and do not affect the top HUD.
